...
void Bytecode::Prototype::operator()(std::vector<Prototype*>& unlinkedPrototypes) {
	...
	read_debug_info();
	assert(prototypeSize == bytecode.fileBuffer.size(), "Prototype has unread bytes left", bytecode.identifierBuilder, DEBUG_INFO);
	unlinkedPrototypes.emplace_back(this);
}

void Bytecode::Prototype::read_header() {
	header.flags = get_next_byte();
	assert(!(header.flags & ~(BC_PROTO_CHILD | BC_PROTO_VARARG | BC_PROTO_FFI)), "Prototype has invalid flags (" + byte_to_string(header.flags) + ")", bytecode.identifierBuilder, DEBUG_INFO);
	...
	assert(instructions.size(), "Prototype has no instructions", bytecode.identifierBuilder, DEBUG_INFO);
...
void Bytecode::Prototype::read_instructions() {
	for (uint32_t i = 0; i < instructions.size(); i++) {
		...
		assert(instructions[i].type < BC_OP_INVALID, "Prototype has invalid instruction (" + byte_to_string(instructions[i].type) + ")", bytecode.identifierBuilder, DEBUG_INFO);
		...
		case BC_OP_FUNCC:
		case BC_OP_FUNCCW:
			assert(false, "Prototype has unsupported instruction (" + byte_to_string(instructions[i].type) + ")", bytecode.identifierBuilder, DEBUG_INFO);
		}
...
void Bytecode::Prototype::read_constants(std::vector<Prototype*>& unlinkedPrototypes) {
	...
	for (uint32_t i = 0; i < constants.size(); i++) {
		...
		case BC_KGC_CHILD:
			...
			assert(unlinkedPrototypes.size(), "Failed to link child prototype", bytecode.identifierBuilder, DEBUG_INFO);
		...
		case BC_KGC_TAB:
			...
			continue;
		case BC_KGC_COMPLEX:
			assert(!get_uleb128() && !get_uleb128(), "Prototype has invalid cdata constant", bytecode.identifierBuilder, DEBUG_INFO);
		case BC_KGC_I64:
		case BC_KGC_U64:
...
void Bytecode::Prototype::read_debug_info() {
	...
	for (uint8_t byte = get_next_byte(); byte; byte = get_next_byte()) {
		...
		assert(scopeOffset != 1, "Prototype variable has invalid scope", bytecode.identifierBuilder, DEBUG_INFO);
	...
	assert(parameterCount == header.parameters, "Prototype parameter count does not\nmatch with debug info", bytecode.identifierBuilder, DEBUG_INFO);
	variableInfos.shrink_to_fit();
}

uint8_t Bytecode::Prototype::get_next_byte() {
	assert(prototypeSize < bytecode.fileBuffer.size(), "Prototype read would exceed end of buffer", bytecode.identifierBuilder, DEBUG_INFO);
